...
function deObjetoAarray(objeto) {
   ...
   for (var i in objeto) {
      arregloDeArreglos.push([i,objeto[i]]);
   }
...
function asAmirror(frase) {
   ...
   for (var i=0; i<fraseSeparada.length; i++) {
      ...
      for(var j= palabra.length-1; j>= 0; j--) {
         palabraReves += palabra[j];
      }
      fraseFinal += palabraReves + " ";
   }
...
function capicua(numero) {
   ...
   if (numeroAlReves === numero.toString()) {
      return "Es capicua";
   } else {
...
function sortArray(arrayOfStrings) {
   ...
   // Tu código:
  var arrayFinal = arrayOfStrings.sort(function(a,b){return a.length - b.length;});;
